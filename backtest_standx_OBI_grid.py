...
def _prepare_context(args: argparse.Namespace) -> BacktestContext | None:
    ...
    tick_size, lot_size, count, converted = convert_parquet_to_npz(
        data_dir, out_path, args.max_rows, args.latency_ns, getattr(args, 'symbol', None), max_days=max_days
    )
    if converted:
...
def _run_single_backtest_worker_args(
    payload: tuple[BacktestContext, float, float, float, float, int],
) -> tuple[float, float, float, int, float | None, int | None]:
    ...
    return float(vol_to_half_spread), float(skew), float(c1_ticks), int(grid_num), equity, num_trades
...
def main() -> None:
    ...
    if args.c1 is not None:
        ...
        c1_ticks = c1_value / ctx.tick_size
        c1_values = [(c1_ticks, c1_value)]
    else:
        c1_ticks_values = _parse_float_list(args.c1_ticks)
        c1_values = [
