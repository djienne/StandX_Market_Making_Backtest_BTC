...
def read_price_files(paths: List[Path]) -> pd.DataFrame:
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        frames = list(executor.map(read_single, paths))
...
def _round_to_significant(value: float, sig_figs: int = 3) -> float:
    """Round to significant figures to handle floating point precision."""
...
def infer_tick_size(price_df: pd.DataFrame, max_levels: int = 10) -> float:
    ...
    bid = price_df[bid_cols].to_numpy(copy=True)
    ask = price_df[ask_cols].to_numpy(copy=True)
    bid[bid <= 0] = np.nan
    ask[ask <= 0] = np.nan
    ...
    # Round to handle floating point precision (e.g., 0.09999999 -> 0.1)
    tick = _round_to_significant(tick, 3)
...
def infer_lot_size(price_df: pd.DataFrame, trade_df: pd.DataFrame, max_levels: int = 10) -> float:
    ...
    if size_cols:
        sizes = price_df[size_cols].to_numpy(copy=True)
        sizes[sizes <= 0] = np.nan
        min_book = _min_positive(sizes)
    else:
        min_book = None

    trade_sizes = trade_df["size"].to_numpy(dtype=np.float64, copy=True)
    ...
    lot = float(min(candidates))
    # Round to handle floating point precision (e.g., 0.00099999 -> 0.001)
    return _round_to_significant(lot, 3)
...
def align_time_window(
    ...
    # max_days: filter to most recent N days (from end of data, going backwards)
    if max_days is not None and max_days > 0:
        ns_per_day = 24 * 60 * 60 * 1_000_000_000
        max_duration_ns = int(max_days * ns_per_day)
        data_end = int(price_df["ts_ns"].max())
        cutoff_start = data_end - max_duration_ns
        price_df = price_df[price_df["ts_ns"] >= cutoff_start]
        trade_df = trade_df[trade_df["ts_ns"] >= cutoff_start]
        if not price_df.empty and not trade_df.empty:
            window_start = max(int(price_df["ts_ns"].min()), int(trade_df["ts_ns"].min()))
            window_end = min(int(price_df["ts_ns"].max()), int(trade_df["ts_ns"].max()))
            price_df = price_df[(price_df["ts_ns"] >= window_start) & (price_df["ts_ns"] <= window_end)]
            trade_df = trade_df[(trade_df["ts_ns"] >= window_start) & (trade_df["ts_ns"] <= window_end)]
...
def build_event_array(
    ...
    # Pre-allocate batch buffer instead of list.append()
    n_snapshots = len(price_ts)
    BATCH_SIZE = 100_000
    max_events_per_snapshot = actual_levels * 4 + 1  # bids + asks + bid_deletes + ask_deletes + gap_clear
    batch_capacity = min(BATCH_SIZE * max_events_per_snapshot, n_snapshots * max_events_per_snapshot)

    all_batches: List[np.ndarray] = []
    batch_arr = np.zeros(batch_capacity, dtype=event_dtype)
    write_idx = 0

    diff = DiffOrderBookSnapshot(actual_levels, tick_size, lot_size)
    depth_ev_base = DEPTH_EVENT
    buy_ev = depth_ev_base | BUY_EVENT
    sell_ev = depth_ev_base | SELL_EVENT
    ...
    for i in range(n_snapshots):
        ...
        if gap_detected is not None and gap_detected[i]:
            batch_arr[write_idx]["ev"] = DEPTH_CLEAR_EVENT
            batch_arr[write_idx]["exch_ts"] = exch_ts
            batch_arr[write_idx]["local_ts"] = local_ts
            write_idx += 1
            diff = DiffOrderBookSnapshot(actual_levels, tick_size, lot_size)
        ...
        # Write bid updates directly to array
        for j in range(curr_bids.shape[0]):
            flag = int(curr_bids[j, 2])
            if flag != CHANGED and flag != INSERTED:
                continue
            batch_arr[write_idx]["ev"] = buy_ev
            batch_arr[write_idx]["exch_ts"] = exch_ts
            batch_arr[write_idx]["local_ts"] = local_ts
            batch_arr[write_idx]["px"] = curr_bids[j, 0]
            batch_arr[write_idx]["qty"] = curr_bids[j, 1]
            write_idx += 1

        # Write ask updates directly to array
        for j in range(curr_asks.shape[0]):
            flag = int(curr_asks[j, 2])
            if flag != CHANGED and flag != INSERTED:
                continue
            batch_arr[write_idx]["ev"] = sell_ev
            batch_arr[write_idx]["exch_ts"] = exch_ts
            batch_arr[write_idx]["local_ts"] = local_ts
            batch_arr[write_idx]["px"] = curr_asks[j, 0]
            batch_arr[write_idx]["qty"] = curr_asks[j, 1]
            write_idx += 1
        ...
        for j in range(bid_delete.shape[0]):
            ...
            batch_arr[write_idx]["exch_ts"] = exch_ts
            batch_arr[write_idx]["local_ts"] = local_ts
            batch_arr[write_idx]["px"] = bid_delete[j, 0]
            batch_arr[write_idx]["qty"] = 0.0
            write_idx += 1
        ...
        for j in range(ask_delete.shape[0]):
            ...
            batch_arr[write_idx]["exch_ts"] = exch_ts
            batch_arr[write_idx]["local_ts"] = local_ts
            batch_arr[write_idx]["px"] = ask_delete[j, 0]
            batch_arr[write_idx]["qty"] = 0.0
            write_idx += 1

        # Flush batch if near capacity
        if write_idx >= batch_capacity - max_events_per_snapshot:
            all_batches.append(batch_arr[:write_idx].copy())
            batch_arr = np.zeros(batch_capacity, dtype=event_dtype)
            write_idx = 0
    ...
    if all_batches:
        depth_arr = np.concatenate(all_batches)
    else:
        depth_arr = np.array([], dtype=event_dtype)
...
def convert_parquet_to_npz(
    ...
    if not quiet:
        print(f"  Data files:      {len(price_paths)} orderbook, {len(trade_paths)} trades")
    ...
    if (
        ...
        and cached.get("max_days") == max_days
    ):
        return (
    ...
    price_df, trade_df = align_time_window(price_df, trade_df, max_rows, max_days)
...
def main() -> None:
    ...
    parser.add_argument("--max-days", type=float, default=7.0, help="Max duration in days from most recent data (default: 7).")
    ...
    tick_size, lot_size, count, converted = convert_parquet_to_npz(
        data_dir, out_path, args.max_rows, args.latency_ns, args.symbol, args.max_levels, args.max_days
    )
