...
def compute_sharpe(records: np.ndarray, annualization: float = 252.0) -> float:
    """Compute annualized Sharpe ratio from backtest records using daily returns.

    Aggregates the equity curve to daily frequency (using end-of-day values),
    computes daily returns, then annualizes with sqrt(252).

    Args:
        records: Backtest records with timestamp, balance, position, price, fee fields.
        annualization: Annualization factor (default 252 for trading days).

    Returns:
        Annualized Sharpe ratio, or 0.0 if insufficient data or invalid result.
    """
    valid_mask = np.isfinite(records["price"])
    ...
    # Convert timestamps to days and aggregate to daily frequency
    timestamps_ns = valid_records["timestamp"]
    # Convert nanoseconds to days (integer day number)
    ns_per_day = 24 * 60 * 60 * 1_000_000_000
    days = timestamps_ns // ns_per_day

    # Get unique days and find the last equity value for each day
    unique_days, day_indices = np.unique(days, return_index=True)
    if len(unique_days) < 2:
        # Not enough days for daily returns, fall back to per-record returns
        ...
        if std_return == 0:
            return 0.0
        # Estimate records per day for proper annualization
        records_per_day = len(equity_curve) / max(1, len(unique_days))
        effective_annualization = annualization * records_per_day
        sharpe = (mean_return / std_return) * np.sqrt(effective_annualization)
        if not np.isfinite(sharpe):
            return 0.0
        return float(sharpe)

    # Get end-of-day equity values efficiently using index boundaries
    # day_indices gives start of each day; end of day i is start of day i+1 minus 1
    end_indices = np.concatenate([day_indices[1:] - 1, [len(equity_curve) - 1]])
    daily_equity = equity_curve[end_indices]

    # Compute daily returns
    daily_returns = np.diff(daily_equity)
    if len(daily_returns) < 2:
        # Only 1 daily return (2 days of data) - fall back to per-record returns
        returns = np.diff(equity_curve)
        if len(returns) == 0 or np.std(returns) == 0:
            return 0.0
        mean_return = np.mean(returns)
